		...
		<div class="am-header">
			...
			<div class="am-top-nav">
				<ul>
					...
					<li class="active button--moema"><a href="fkzzfxAction!*index2" style="color:#dadc37;">导弹需求</a></li>
												...
												<tr>
													<td style="height:111px !important">
														<div class="circle-input m-t-10 m-b-10 bc_blue_20">
															<input type="text" value="" class="t-center" disabled id="dkrwl">
														</div>
													</td>
												</tr>
											</tbody>
												...
												<tr>
													<td style="height:111px !important">
														<div class="circle-input m-t-10 m-b-10">
															<input type="text" value="0.8" class="t-center" id="hszb">
														</div>
													</td>
												</tr>
											</tbody>
												...
												<tr>
													<td style="height:111px !important">
														<div class="circle-input m-t-10 m-b-10 bc_blue_20" style="width:200px;">
															<input type="text" value="HQ17战车" class="t-center" style="width:200px;" disabled id="bzdkhldy"> 
														</div>
													</td>
												</tr>
											</tbody>
										...
										<table class="am-table am-table-bordered">
											<tbody>
												...
												<tr>
													<td style="height:111px !important">
														<div class="circle-input m-t-10 m-b-10 bc_blue_20">
															<input type="text" value="" class="t-center" disabled id="dkllxql">
										...
										<table class="am-table am-table-bordered">
											<tbody>
												...
												<tr>
													<td style="height:111px !important">
														<div class="circle-input m-t-10 m-b-10 bc_blue_20">
															<input type="text" value="" class="t-center" disabled id="dkryl">
														</div>
													</td>
												</tr>
											</tbody>
												...
												<tr>
													...
													<td  class="bar-css">
														<div style="height:125px;" class="bar-box bg-color-green t-center">
															<div class="bar-box-p" style="font-size:20px;" id="dksjxql"></div>
														</div>
													</td>	
												</tr>
											</tbody>
					...
					<div class=" border-polygon-lg height-2">
						<div class="am-box">
							<div class="am-box-title">
								...
								<p>需求还原计算</p>
								<div class="am-btn-small pull-right m-l-20 m-t-5" id="bc">保存</div>
								<div class="am-btn-small pull-right m-l-20 m-t-5" id="js2">计算</div>
								<div class="am-btn-small pull-right m-t-5" data-modal="#tips-value-3" data-modal-init>加载</div>
								...
								<table class="am-table am-table-bordered am-table-striped am-table-hover ">
									<thead>
										<tr>
											...
											<th width="10%" rowspan="2" class="t-center">地空导弹型号</th>
											<th width="%" colspan="5" class="t-center">地空导弹火力单元</th>
											<th width="%" colspan="6" class="t-center">地空导弹弹药</th>
										</tr>
										<tr>
											<th width="8%" class="t-center">作战效能标准值</th>
											<th width="8%" class="t-center">任务比例</th>
											<th width="8%" class="t-center">配备数量</th>
											<th width="8%" class="t-center">需求数量</th>
											<th width="8%" class="t-center">资源满意度</th>
											<th width="8%" class="t-center">直接损耗率</th>
											<th width="8%" class="t-center">自然损耗率</th>
											<th width="8%" class="t-center">冗余率</th>
											<th width="8%" class="t-center">库存数量</th>
											<th width="8%" class="t-center">需求数量</th>
											<th width="8%" class="t-center">资源满意度</th>
										</tr>
									</thead>
			...
			<div class="dialog-content-2">
				<div class="dialog-header">
					<div class="dialog-header-left">
						<i class="fa fa-fire c_7af5ff"></i><span>地空导弹 </span> 
					</div>
					...
					<div class="m-t-20 pull-left amCheckAllBox" style="width:100%;">
						<table class="am-table am-table-bordered am-table-striped am-table-hover"  style="width:100%; height:40px;">
							<thead>
								<tr>
									...
									<th class="t-center" width="13%">地空导弹型号</th>
									<th class="t-center" width="13%">作战效能标准值</th>
									<th class="t-center" width="13%">单发毁伤概率</th>
						...
						<div class="src-bar" style="width:100%;max-height:288px;padding:0;margin:0;">
							<table class="am-table am-table-bordered am-table-striped t-center"  style="padding:0;margin:0;">
								<tbody>
									<c:forEach items="${pzcss3}" var="p" varStatus="c">
									<tr>
										<td width="3%" class="smart-form">
											...
											<input type="hidden" value="${p.dkddbh}" />
										</td>
										<td width="6%">${c.index+1}</td>
										<td width="13%">${p.dkddmc}</td>
										<td width="13%">${p.bzxnzs}</td>
										<td width="13%">${p.dfhsgl}</td>
			...
			$(function(){
				$.ajax({
					...
					'success' : function(data) {
						$("#dkrwl").val((Number(data[4].csz)*Number(data[7].csz)).toFixed(2));
						$("#hszb").val(data[14].csz);
						$("#yxzsl").val(data[17].csz);
						$("#ybybl").val(data[19].csz);
						$("#bzdkhldy").val(data[2].cslg);
						$("#zbkkx").val(data[18].csz);
						circleDom('','circleDom_2_1',(data[15].csz)*100); //第2方案
						
						$("#kxgm").val(data[4].csz);
						$("#ztkjl").val(data[5].csz);
						$("#rwbl").val(data[7].csz);
						$("#xzxs").val(data[16].csz);
					}
				});
				...
				$("#qd").on("click",function(){
					...
					for(var i=1;i<checkLen;i++){
						if($(".smart-form").eq(i).find("input[type=checkbox]")[0].checked){
							dkbh.push($(".smart-form").eq(i).find("input[type=hidden]").val());
						}
					}
					
					for(var i=0;i<dkbh.length;i++){
						$.ajax({
							'url' : 'fkzzfxAction!*findDkBh?bh='+dkbh[i],
							'type' : 'post',
							'dataType' : 'json',
							'async' : false,
							'success' : function(data) {
								var tr = '<tr>'+
											...
											'<td width="8%" class="bc_blue_10"></td>'+
											'<td width="8%" class="bc_blue_10"></td>'+
											'<input type="hidden" value="'+data.dfhsgl+'" class="dcdshsgl"/>'+
										'</tr>';
								$("#tbody").append(tr);
							}
						});
				...
				$("#js2").on("click",function(){
					...
					if(blCount.toFixed(1) == 1.0){
						for(var i=0;i<length;i++){
							...
							var kjzb = Number($("#ztkjl").val())*Number($("#rwbl").val());
							var mi = Number($("#dksjxql").html())*Number($("#jdt_"+i).text());
							var m = Number($("#dksjxql").html());
				...
				$("#bc").on("click",function(){
					...
					for(var i=0;i<length;i++){
						var ddbc = {};
						ddbc.id = $("#tbody").find("tr").eq(i).find("td").eq(0).html();
						ddbc.xh = $("#tbody").find("tr").eq(i).find("td").eq(1).html();
						ddbc.rwbl = Number($("#jdt_"+i).text());
						ddbc.hlxq = Number($("#tbody").find("tr").eq(i).find("td").eq(5).html());
						ddbc.hlmyd = Number($("#tbody").find("tr").eq(i).find("td").eq(6).html().split("%")[0])/100;
						ddbc.dyxq = Number($("#tbody").find("tr").eq(i).find("td").eq(11).html());
						ddbc.dymyd = Number($("#tbody").find("tr").eq(i).find("td").eq(12).html().split("%")[0])/100;
						ddsz.push(ddbc);
					}
					var ddz = JSON.stringify(ddsz);
					console.log(ddz);
					$.ajax({
						url:'fkzzfxAction!*insertDd?ddz='+ddz,
						type:'get',
						success : function(data){
							alert("保存成功!");
						},
